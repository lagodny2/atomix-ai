...
async function loadPromptFile(mode) {
    ...
    try {
        ...
        const cached = promptCache.get(pathToPrompt);
        if (cached && cached.mtimeMs === stat.mtimeMs) return cached.prompt;

        const raw = await fs.readFile(pathToPrompt, 'utf8');
        const corePromptMatch = raw.match(/# SECURITY CORE PROMPT[\s\S]*$/);
        const corePrompt = corePromptMatch ? corePromptMatch[0].trim() : '';
...
export default async (request) => {
    ...
    const activeSecurityPrompt = await loadPromptFile(securityMode);
    let webContext = "";
        ...
        effectiveImages.length
            ? {
                ...
                content: [
                    ...
                    ...effectiveImages.map((image) => ({ type: "image_url", image_url: { url: image.dataUrl } }))
                ]
            }
